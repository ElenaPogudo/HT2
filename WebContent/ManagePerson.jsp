    ...
    <tr>
        ...
        <td>
         <%  for(String phone : person.getPhones().values())
            ...
            out.write(phone + "\n");
         %>
            <a href="<%=request.getContextPath()%>/?action=edit_number&id=<%=person.getId()%>">Редактировать</a>
            <a href="<%=request.getContextPath()%>/?action=delete_number&id=<%=person.getId()%>">Удалить</a><br>
            <%}%>
            <a href="<%=request.getContextPath()%>/?action=add_number&id=<%=person.getId()%>">Добавить</a>
        </td>
    </tr>
    <tr>
        ...
        <td colspan="2" align="center">
         <input type="submit" name="<%=jsp_parameters.get("next_action")%>" value="<%=jsp_parameters.get("next_action_label")%>" />
            <br><a href="<%=jsp_parameters.get("next_action_label")%>"> Вернуться к списку</a>
        </td>
    </tr> 
 </table>
 </form>
